...
async def upload_image(
    ...
    subfolder = _build_subfolder(scope=scope, project_id=project_id, board_id=board_id)
...
def _build_subfolder(scope: str, project_id: int | None, board_id: int | None) -> str:
    if scope in {"document", "note", "comment"}:
        if project_id is None or project_id <= 0:
            raise HTTPException(status_code=400, detail="project_id가 필요합니다.")
        return f"editor_images/projects/{project_id}/{scope}"
