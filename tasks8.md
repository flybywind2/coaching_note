...
- [ ] 저장 버튼 클릭 시 설문 작성값 DB 저장 보장
  - 후보 파일: `backend/app/routers/surveys.py`, `backend/app/services/survey_service.py`
- [ ] 제출 버튼 동작 반영
  - `Summitted=True` 저장
  - 제출취소 버튼 노출
- [ ] 관리자 결과 기준을 제출완료(`Summitted=True`)로 통일
...
- [ ] `feedback8.md` 작성/업데이트
  - 기능별 수정 파일/핵심 로직/검증 결과 기록
- [ ] 코드 내 `[feedback8]` 주석 누락 점검
  - `rg "\[feedback8\]" backend frontend`
- [ ] 통합 회귀 테스트 및 수동 시나리오 검증 결과 반영

## 3. 권장 구현 순서(실행 계획)

### Phase A. 기반 정리

- [ ] `[feedback8]` 주석 규칙 적용 범위 정의
- [ ] `feedback8.md` 템플릿 생성
- [ ] 강의/설문 기존 테스트 기준선 확인
...
- [ ] 기능 커밋마다 Chrome DevTools로 실제 시나리오 점검
- [ ] 신규/수정 테스트 통과 확인
- [ ] `feedback8.md` 최종 업데이트 후 완료 기준 점검
...
- [ ] 강의리스트/강의상세 화면이 분리되어 동작함
- [ ] 강의리스트 카드에 요구 지표와 참여자 신청여부가 표시됨
- [ ] 강의상세 지표가 한 줄 배치되고 승인인원은 관리자에게만 노출됨
- [ ] 관리자가 캘린더에서 강의 내용을 편집할 수 있음
- [ ] 강의 시간 설정이 10분 단위로 검증됨(UI + 서버)
- [ ] 설문 구성/결과 화면이 분리되고 코치 결과 조회가 가능함
- [ ] 기존 질문 재활용으로 신규 설문 생성이 가능함
- [ ] 설문 저장/제출/제출취소가 `Summitted` 상태와 정확히 동기화됨
- [ ] `feedback8_changed.md`에 변경 파일/내용/검증 결과가 기록됨
- [ ] 변경 코드에 `[feedback8]` 주석이 일관되게 추가됨
