...
- 리치 에디터 확장: 이미지 외 일반 파일 첨부 업로드 지원, 게시글/코칭노트 첨부 렌더링 보강
- 멘션 개선: `@이름` 입력 시 역할 접두어(예: `코치 홍길동`)가 있는 사용자도 자연어 이름으로 매칭되도록 보정
- 시간 표시 통일: 프론트 포맷터를 `KST(Asia/Seoul)` 기준으로 통일
- 코치 소개 보강: 차수 코치 목록 생성 시 `UserBatchAccess`뿐 아니라 과제 멤버(`ProjectMember`) 기반 코치도 fallback 노출
- 코칭 계획/실적 UX 개편:
  - 기본 화면을 코치별 월 단위 카드형 관리로 제공하고, 기존 전체 그리드 보기 병행
  - 계획 입력에서 과제 선택 제거(계획은 시간/메모 중심)
  - 실적 입력에서 과제 다중 선택 지원 및 실적 셀에 과제명 표시
  - 코치도 본인 실적 보정 입력/삭제 가능(관리자는 전체)
  - 기본 월은 이번 달로 자동 선택
  - 날짜 필터 추가: 기본 `코칭일정만`, 필요 시 `전체` 전환
  - `코칭일정만`에서는 월 선택을 숨기고 차수 전체 범위에서 코칭일정 날짜만 표시
- 캘린더 확장:
  - 공개 범위에 `코칭 일정(coaching scope)` 추가 (`global`/`coaching`/`project`)
  - 코칭 일정 상세에 코치 계획/실적(자동/수동) 연동 정보 노출
  - 참여자는 코칭 일정 비노출, 참관자는 권한 과제 이벤트 + 코칭 일정 조회 가능(쓰기 권한 없음)
- 대시보드 정합성 개선:
  - 출석 현황/코칭노트·코칭의견 지표를 코칭 계획/실적과 동일하게 `코칭일정만` 필터(기본) + `전체` 토글로 표시
  - 필터 적용 시 합계/비율도 필터된 날짜 기준으로 재계산
...
- 협업 강화(P2): 코칭노트 템플릿 저장/재사용, 코칭노트/문서/게시글 변경 이력 조회/복원, 멘션(`@`) 알림 연동(이름 매칭 보강)
- 문서/게시판: 리치 에디터 기반 편집(표/이미지/파일 첨부/링크/HTML 모드), 게시글/댓글 편집 및 삭제, 공지사항 게시판 작성 관리자 제한
- Task/마일스톤: 생성/편집/삭제, 상태 변경, 마일스톤 순서/캘린더 연동, Task 담당자 지정(해당 과제 팀원만 가능)
- 세션/출석: 세션 관리, 출석 체크인/체크아웃, 코칭 시작/종료 로그
- 캘린더/대시보드: 월간 뷰 + 10주 뷰, 프로젝트별 마일스톤 시각화, 공개범위(`global/coaching/project`) 일정 관리, 코칭일정 기준 지표 필터 대시보드
- 소개 페이지: SSP+ 소개/코치 소개 콘텐츠 관리, 코치 프로필 CRUD
- 코칭 계획: 코치별 월간/그리드 조회, 계획 시간·메모 입력, 실적 보정(복수 과제 선택) 입력
...
| Schedule | `POST/PUT/DELETE /api/schedules...` | `admin` (`visibility_scope`: `global`/`coaching`) |
| Calendar | `GET /api/calendar` | 로그인 사용자 (참여자: 본인 과제 + 공통, 참관자: 권한 과제 + 공통 + 코칭) |
...
| Coaching Plan | `/api/coaching-plan/...` | `admin`, `coach` (코치는 본인 계획/실적만) |
