...
Pages.dashboard = {
  async render(el, params) {
    ...
    try {
      ...
      const mode = State.get('dashMode') || 'progress';
  ...
  _renderMatrix({ mode, projects, dates, attendanceRows, noteRows }) {
    if (!projects.length) {
    ...
    if (mode === 'progress') {
      return `<div class="dash-matrix-wrap" id="dash-matrix-wrap"><table class="data-table dash-matrix-table">
        <thead>
          <tr>
            ...
            return `
              <tr>
                <td class="sticky-col sticky-col-project">
                  ...
                  <div class="dash-subtext">대상 ${Number(project.expected_count || 0)}명</div>
                </td>
                <td class="sticky-col sticky-col-total dash-total-cell">${Math.max(0, Math.min(100, progress)).toFixed(0)}%</td>
                <td class="dash-value-cell">
                  <div class="dash-progress-bar"><span class="dash-progress-fill" style="width:${Math.max(0, Math.min(100, progress))}%;"></span></div>
                </td>
              </tr>
            `;
          }).join('')}
    ...
    return `<div class="dash-matrix-wrap" id="dash-matrix-wrap"><table class="data-table dash-matrix-table">
      ...
      <tbody>
        ${projects.map((project) => {
          if (mode === 'attendance') {
            const row = attendanceMap.get(project.project_id) || { cells: [], expected_count: Number(project.expected_count || 0) };
            ...
            return `
              <tr>
                <td class="sticky-col sticky-col-project">
                  <a href="#/project/${project.project_id}" class="proj-link">${Fmt.escape(project.project_name)}</a>
                  <div class="dash-subtext">대상 ${expectedCount}명</div>
                </td>
